...
$listProcess = Start-Process -FilePath $vlcExe -ArgumentList "--intf","none","--no-interact","--list" -PassThru -NoNewWindow -RedirectStandardOutput $listStdout -RedirectStandardError $listStderr
...
if (-not $moduleFound -and -not $listTimedOut) {
...
# Check preliminary results - fail early only on definite failures
if ($headlessTestFailed) {
    ...
    exit 1
} elseif (-not $moduleFound -and -not $loadSuccess -and -not $listTimedOut) {
    # Only fail if --list ran successfully but module wasn't found
    ...
    Write-Host "Plugin load check: SKIPPED (VLC --list timed out)" -ForegroundColor Yellow
    Write-Host "Proceeding to integration test..." -ForegroundColor Yellow
} else {
