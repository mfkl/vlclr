...
// Used to return rendered subtitle images to VLC
...
/// Creates subpicture regions suitable for returning from the text renderer.
/// </summary>
public static class PictureConverter
{
    ...
    /// </summary>
    /// <param name="image">The rendered subtitle image (RGBA format).</param>
    ...
    public static unsafe nint ToSubpictureRegion(Image<Rgba32> image, nint chromaListPtr)
    {
        ...
        if (chroma == 0)
        {
            Console.Error.WriteLine("[.NET Subtitle] No supported chroma format found");
            return nint.Zero;
        }
        ...
        {
            Console.Error.WriteLine("[.NET Subtitle] Failed to create picture");
            return nint.Zero;
        }
        ...
        if (!copySuccess)
        {
            Console.Error.WriteLine("[.NET Subtitle] Failed to copy pixels to picture");
        ...
        {
            Console.Error.WriteLine("[.NET Subtitle] Failed to create subpicture region");
            VLCCore.PictureDestroy(picturePtr);
            return nint.Zero;
        }
    ...
    /// </summary>
    private static unsafe uint SelectChroma(nint chromaListPtr)
    {
    ...
    /// </summary>
    private static VLCVideoFormat CreateFormat(uint chroma, uint width, uint height)
    {
    ...
    /// </summary>
    private static unsafe bool CopyPixelsToPicture(Image<Rgba32> image, nint picturePtr, uint chroma)
    {
