/**
 * .NET Bridge Header
 * Function pointer declarations for dynamically loaded .NET Native AOT exports.
 */
...
typedef int (*dotnet_open_fn)(void* vlc_object);
typedef void (*dotnet_close_fn)(void* vlc_object);

/* Function pointers resolved at runtime */
extern dotnet_open_fn dotnet_plugin_open;
extern dotnet_close_fn dotnet_plugin_close;
 ...
 * @param vlc_object Pointer to vlc_object_t
 * @param name Variable name
 * @param buffer Buffer to receive the string (caller must free with dotnet_bridge_free_string)
...
/**
 * Free a string returned by dotnet_bridge_var_get_string.
 ...
 */
typedef enum {
    DOTNET_PLAYER_STATE_STOPPED = 0,
    DOTNET_PLAYER_STATE_STARTED,
    DOTNET_PLAYER_STATE_PLAYING,
    DOTNET_PLAYER_STATE_PAUSED,
    DOTNET_PLAYER_STATE_STOPPING,
} dotnet_player_state_t;

/**
 * Player event callback types for C#.
 * These are called from the C glue layer when VLC events occur.
 */
typedef void (*dotnet_on_state_changed_fn)(int new_state, void* user_data);
typedef void (*dotnet_on_position_changed_fn)(long long new_time, double new_pos, void* user_data);
typedef void (*dotnet_on_media_changed_fn)(void* new_media, void* user_data);

/**
 * Player listener callbacks structure for C#.
 */
typedef struct {
    dotnet_on_state_changed_fn on_state_changed;
    dotnet_on_position_changed_fn on_position_changed;
    dotnet_on_media_changed_fn on_media_changed;
    void* user_data;
} dotnet_player_callbacks_t;
 ...
 * @return Current player state (dotnet_player_state_t values)
 */
 ...
 * @param player Pointer to vlc_player_t
 * @param callbacks Pointer to dotnet_player_callbacks_t structure
 ...
 * @param player Pointer to vlc_player_t
 * @param listener_id Listener ID returned by dotnet_bridge_player_add_listener
 */
BRIDGE_API void dotnet_bridge_player_remove_listener(void* player, void* listener_id);
 ...
 */
typedef enum {
    DOTNET_SEEK_PRECISE = 0,  /* Seek to exact time (may be slower) */
    DOTNET_SEEK_FAST = 1,     /* Seek to nearest keyframe (faster) */
} dotnet_seek_speed_t;
 ...
 */
typedef enum {
    DOTNET_SEEK_ABSOLUTE = 0, /* Seek from beginning of media */
    DOTNET_SEEK_RELATIVE = 1, /* Seek relative to current position */
} dotnet_seek_whence_t;
