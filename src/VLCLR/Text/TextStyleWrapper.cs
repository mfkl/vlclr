// Text style wrapper for VLC text_style_t
...
/// </summary>
/// <remarks>
/// This class converts native VLC text style structures to C# properties.
/// It handles null style pointers gracefully by returning default values.
/// </remarks>
public sealed class TextStyleWrapper
{
    ...
    /// <summary>
    /// Creates a TextStyleWrapper from a native VLC text_style_t pointer.
    /// </summary>
    /// <param name="stylePtr">Pointer to VLCTextStyle struct, or nint.Zero for defaults.</param>
    /// <returns>TextStyleWrapper with values from native struct or defaults.</returns>
    public static unsafe TextStyleWrapper FromNative(nint stylePtr)
    {
        if (stylePtr == nint.Zero)
        {
            return new TextStyleWrapper();
        }
    ...
    /// <summary>
    /// Creates a TextStyleWrapper with visibility-optimized defaults.
    /// Forces white text with outline for visibility on dark videos.
    /// </summary>
    /// <param name="stylePtr">Pointer to VLCTextStyle struct, or nint.Zero for defaults.</param>
    ...
    /// <returns>TextStyleWrapper with visibility-optimized values.</returns>
    public static unsafe TextStyleWrapper FromNativeWithVisibility(
        nint stylePtr, 
        ...
        int outlineWidth = 3)
    {
        if (stylePtr == nint.Zero)
        {
            return new TextStyleWrapper
            {
                HasOutline = forceOutline,
                OutlineWidth = forceOutline ? outlineWidth : DefaultOutlineWidth
            };
        }
        ...
        var fgColor = (forceWhiteText && style.FontColor == 0x000000) ? 0xFFFFFFu : style.FontColor;

        // Always enable outline for visibility
        var hasOutline = forceOutline || (flags & VLCTextStyleFlags.Outline) != 0;
        var effectiveOutlineWidth = forceOutline ? outlineWidth : 
            (style.OutlineWidth > 0 ? style.OutlineWidth : DefaultOutlineWidth);
    ...
    /// <summary>
    /// Extracts the red component from a 0xRRGGBB color.
    /// </summary>
    public static byte GetRed(uint color) => (byte)((color >> 16) & 0xFF);

    /// <summary>
    /// Extracts the green component from a 0xRRGGBB color.
    /// </summary>
    public static byte GetGreen(uint color) => (byte)((color >> 8) & 0xFF);

    /// <summary>
    /// Extracts the blue component from a 0xRRGGBB color.
    /// </summary>
    public static byte GetBlue(uint color) => (byte)(color & 0xFF);

    /// <summary>
    /// Creates a 0xRRGGBB color from RGB components.
    /// </summary>
    public static uint MakeColor(byte r, byte g, byte b) => ((uint)r << 16) | ((uint)g << 8) | b;
