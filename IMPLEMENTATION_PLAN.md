...
VLC 4.x Plugin System
        ...
        ▼                                    ▼
┌──────────────────────────┐    ┌────────────────────────────┐
│  libdotnet_bridge_plugin │    │  libdotnet_filter_plugin   │
│  capability: "interface" │    │  capability: "video filter"│
│  plugins/control/        │    │  plugins/video_filter/     │
└──────────────────────────┘    └────────────────────────────┘
        │                                    │
        └─────────────┬──────────────────────┘
                      │ Both load same VlcPlugin.dll
                      ▼
┌──────────────────────────────────────────────────────────┐
│  VlcPlugin.dll (Native AOT)                              │
│  - DotNetPluginOpen/Close (interface plugin)             │
│  - DotNetFilterOpen/Close/Frame (video filter plugin)    │
│  - ImageSharp text rendering                             │
└──────────────────────────────────────────────────────────┘
```
...
└── plugins/
    ├── control/
    │   ├── libdotnet_bridge_plugin.dll   ← Interface plugin (existing)
    │   └── VlcPlugin.dll                 ← C# Native AOT (shared)
    └── video_filter/
        ├── libdotnet_filter_plugin.dll   ← Video filter plugin
        └── VlcPlugin.dll                 ← Copy of C# DLL (for DLL loading)
```
